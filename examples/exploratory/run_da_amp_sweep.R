#' =======================================================================
#' DA Analysis + Amplitude Fold-Change Sensitivity Sweep
#' =======================================================================
#'
#' Runs only:
#'   1. DA power analysis (amp_diff = c(2,4), plotWithSE_panelC160 to n=160)
#'   2. Amplitude fold-change sweep (fixed c(fc,fc) for fc in 1.25,1.5,2,3,4)
#'
#' Output saved to: output/run_panelC160/
#'
#' USAGE:
#'   Rscript examples/run_da_amp_sweep.R
...
# =====================================================================
# PLOTTING FUNCTION (Panel C extended to n=160)
# =====================================================================
...
# =====================================================================
# DA ANALYSIS (extended to n=160)
# =====================================================================

cat("====================================================================\n")
cat("ANALYSIS 1: DIFFERENTIAL AMPLITUDE (DA)\n")
cat("====================================================================\n\n")
...
# =====================================================================
# AMPLITUDE FOLD-CHANGE SENSITIVITY SWEEP
# =====================================================================

cat("\n====================================================================\n")
cat("ANALYSIS 2: AMPLITUDE FOLD-CHANGE SENSITIVITY\n")
cat("====================================================================\n\n")

amp_fcs <- c(1.25, 1.5, 2, 3, 4)
amp_fc_results <- vector("list", length(amp_fcs))
names(amp_fc_results) <- paste0(amp_fcs, "x")

for (i in seq_along(amp_fcs)) {
  fc <- amp_fcs[i]
  cat(sprintf("  Running DA sweep: fold-change = %.2fx\n", fc))
  ...
  amp_fc_results[[i]] <- runPowerAnalysis(opts_bio_fc, opts_design, opts_analysis,
                                          test_type = "DA")
}

afc_results_file <- file.path(base_out, "da_amp_fc_sweep_results.rds")
saveRDS(amp_fc_results, file = afc_results_file)
cat(sprintf("\nAmplitude fold-change sweep saved: %s\n", afc_results_file))
