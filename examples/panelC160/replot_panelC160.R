# Replot DR/DP with Panel C extended to n=160 using existing results.
# Usage:
#   Rscript examples/replot_panelC160.R
...
dr_path <- file.path(base_out, "dr_power_raw_pvalues.rds")
if (file.exists(dr_path)) {
  load(dr_path)
  dr_fig <- file.path(out_dir, "dr_power.pdf")
  plotWithSE_panelC160(dr_power_raw, dr_fig, test_name = "DR")
}

dp_path <- file.path(base_out, "dp_power_raw_pvalues.rds")
if (file.exists(dp_path)) {
  load(dp_path)
  dp_fig <- file.path(out_dir, "dp_power.pdf")
  plotWithSE_panelC160(dp_power_raw, dp_fig, test_name = "DP")
}
