#' Test the fixed DP phase implementation
...
cat("Running debug simulation with FIXED phase test...\n")
...
if (length(null_idx) > 0) {
  ...
  cat("P < 0.10:", sum(pvals_null < 0.10), "(", round(100*sum(pvals_null < 0.10)/length(pvals_null), 1), "%)\n")
  cat("P > 0.50:", sum(pvals_null > 0.50), "(", round(100*sum(pvals_null > 0.50)/length(pvals_null), 1), "%)\n")

  # Check uniform distribution of p-values for nulls (should be roughly uniform)
  cat("\nExpected: P-values for null genes should be UNIFORM\n")
  cat("If p < 0.05 is close to 5%, the test is working correctly\n")
}
...
if (length(target_idx) > 0) {
  ...
  cat("P < 0.10:", sum(pvals_target < 0.10), "(", round(100*sum(pvals_target < 0.10)/length(pvals_target), 1), "%)\n")
}

cat("\n=== SUMMARY ===\n")
cat("If null genes have ~5% p < 0.05, the test is calibrated correctly\n")
cat("If target genes have high power (many p < 0.05), the test is sensitive\n")
