...
# ---------------------------------------------------------------------
# 1. detect_cosinor(K = 1) type-I error (single-harmonic F-test)
# ---------------------------------------------------------------------
cat("\n=== detect_cosinor K=1 type-I error (single-harmonic F-test) ===\n")
fail_DCP <- 0L
for (n in N_VALUES) {
  r <- .run_null(n, N_SIMS,
                 function(M, t) detect_cosinor(M, t, K = 1L, period = PERIOD),
                 seed = SEED + n)
  s <- .summary("cosinor K=1", n, r$pvals)
