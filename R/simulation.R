...
#' on \code{bio.opts} by \code{\link{estCircadianParam2H}}. The same index is
#' drawn across all five vectors so the pilot's cross-parameter dependence is
#' preserved.
#'
#' \code{r_values} is reported on the 1st-harmonic scale \eqn{r_g = A_{g,1} / \sigma_g}
#' to match the existing r-stratification machinery and the manuscript's
#' \eqn{r_g} notation.
#'
#' @param bio.opts A \code{CircadianBioOptions} produced by
#'   \code{\link{estCircadianParam2H}} (must carry \code{paired_2h = TRUE},
#'   \code{amplitude2}, \code{phase2}).
#' @param cts Numeric vector of sample collection times in hours; length
#'   determines \eqn{N}.
...
#'     \item{\code{is_rhythmic}}{Logical vector.}
#'     \item{\code{r_values}}{Per-gene 1st-harmonic SNR \eqn{A_{g,1}/\sigma_g}.}
#'     \item{\code{A1, phi1, A2, phi2}}{Per-gene truth vectors (length
...
simCircadianSingleCohort2H <- function(bio.opts, cts, seed = NULL) {
  stopifnot(inherits(bio.opts, "CircadianBioOptions"))
  if (!isTRUE(bio.opts$paired_2h) ||
      is.null(bio.opts$amplitude2) || is.null(bio.opts$phase2))
    stop("simCircadianSingleCohort2H requires bio.opts produced by ",
         "estCircadianParam2H() (paired_2h = TRUE, amplitude2/phase2 present).")
  ...
  # --- joint 5-way pilot draw, paired by gene index ---
  ...
  if (is.null(sig_src) || length(sig_src) != L_pilot)
    stop("bio.opts$sigma_rhythmic must be paired (same length as amplitude) ",
         "for simCircadianSingleCohort2H.")

  if (n_rhythmic > 0L) {
    ji <- sample.int(L_pilot, n_rhythmic, replace = TRUE)
    amp_g[rhythmic_id]   <- pmax(amp1_src[ji], 0.05)
    phase_g[rhythmic_id] <- phi1_src[ji]
    A2_g[rhythmic_id]    <- pmax(amp2_src[ji], 0)
    phi2_g[rhythmic_id]  <- phi2_src[ji]
    sigma_g[rhythmic_id] <- pmax(sig_src[ji], 1e-6)
  }

  # 1st-harmonic SNR per manuscript notation
  r_values <- amp_g / sigma_g
  ...
  for (g in seq_len(ngenes)) {
    if (is_rhythmic[g]) {
      mu <- mesor_g[g] +
            amp_g[g] * cos(omega_circ * (cts - phase_g[g])) +
            A2_g[g]  * cos(2 * omega_circ * (cts - phi2_g[g]))
    } else {
      mu <- rep(mesor_g[g], N)
    }
    expr[g, ] <- rnorm(N, mu, sigma_g[g])
  ...
  list(expr = expr, is_rhythmic = is_rhythmic, r_values = r_values,
       A1 = amp_g, phi1 = phase_g, A2 = A2_g, phi2 = phi2_g)
}
