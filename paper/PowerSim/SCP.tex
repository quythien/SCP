...
We present \textsc{SCP} (Simulation-based Circadian Power), a unified framework for sample-size planning in circadian transcriptomic studies. \textsc{SCP} provides closed-form power calculations for circadian biomarker detection, building upon \textsc{CircaPower}, and uses pilot-data-driven simulation to evaluate power and false discovery rate (FDR) for differential circadian analysis under both active and passive designs. A bootstrap procedure quantifies uncertainty in power curves and sample-size recommendations.
...
correction at nominal FDR level $\alpha$ controls the false discovery rate for
rhythmicity detection.

\paragraph{CircaPower closed-form calculation.}
Under the cosinor model, the noncentrality parameter of the $F$-test is
\begin{equation}
\lambda_g = r_g^2 \cdot n \cdot d(\phi_g),
\label{eq:ncp}
\end{equation}
where the sampling design factor $d(\phi_g)$ captures how well the collection
times cover the gene's peak expression time:
\begin{equation}
d(\phi_g)
=
...
\paragraph{Limitation of the single-$r$ assumption.}
\textsc{CircaPower} applies \eqref{eq:circapower_formula} at a single
representative $r$ specified by the user. In transcriptome-wide studies, $r_g$
varies substantially across genes: core clock genes in mouse liver typically
have $r \approx 3$--$6$, while the median $r$ across all expressed genes in
human brain tissue is $\tilde{r}\approx 0.6$ \citep{zong2023circapower}. Using
a single high $r$ therefore produces severely over-optimistic power projections
for the majority of the transcriptome. Moreover, the per-gene formula
\eqref{eq:circapower_formula} does not account for the multiple-testing
correction applied when testing thousands of genes simultaneously.

\paragraph{SCP extension: pilot-calibrated transcriptome-level power.}
When a pilot dataset is available, \textsc{SCP} estimates gene-level
parameters directly from the data. For each of the $G_0$ pilot genes, we fit
\eqref{eq:cosinor_model} by OLS, yielding estimates
$(\hat{\mu}_g, \hat{A}_g, \hat{\sigma}_g, \hat{\phi}_g)$ and a BH-adjusted
rhythmicity $q$-value $\hat{q}_g$. The rhythmic gene set is
$\mathcal{G}_R = \{g : \hat{q}_g < 0.10\}$.

\textsc{SCP} supports two modes for single-cohort power analysis:

\begin{enumerate}
\item \textbf{Closed-form (fast):} Apply \eqref{eq:circapower_formula} to each
  pilot gene's estimated $\hat{r}_g = \hat{A}_g/\hat{\sigma}_g$ and design factor
  $\hat{d}(\hat{\phi}_g)$, then average across genes:
  \[
  \widehat{\mathrm{Power}}_{\mathrm{CF}}(n)
  =
  \frac{1}{G_0}\sum_{g=1}^{G_0} \mathrm{Power}_g(n;\,\hat{r}_g,\hat{d}_g).
  \]
  This is fast and directly extends \textsc{CircaPower} to heterogeneous $r$,
  but does not model genome-wide FDR.

\item \textbf{Simulation-based (recommended):} Simulate single-group datasets
  from the pilot-estimated empirical distributions, apply the $F$-test with BH
  correction, and aggregate empirical power and FDR across replicates (Step~I
  and the data simulation step below). Simulation naturally captures
  genome-wide multiplicity correction, the full joint heterogeneity of
  $(A_g, \sigma_g)$, and passive-design timing variability — features that
  are not accessible to the closed-form approach.
\end{enumerate}

\paragraph{Step I: Pilot parameter estimation.}
From the pilot gene fits, we construct empirical distributions:
\begin{equation}
\hat{F}_\mu
=
...
dependence between amplitude and noise, so the induced distribution of
$r=A/\sigma$ reflects the pilot transcriptome without requiring $r$ to be
specified separately. We denote the full pilot summary by
$\hat{\Psi} = (\hat{F}_\mu, \hat{F}_{A,\sigma}, \hat{F}_\phi,
\hat{F}_{\mathrm{TOD}}, \hat{\pi}_R)$,
where $\hat{F}_{\mathrm{TOD}}$ is the empirical collection-time distribution
(for passive designs) and $\hat{\pi}_R = |\mathcal{G}_R|/G_0$.

\paragraph{Data simulation.}
For simulation replicate $s$ and candidate sample size $n$, we generate
$G$ genes as follows. Draw $\mu_g^{(s)}\sim\hat{F}_\mu$,
$(A_g^{(s)},\sigma_g^{(s)})\sim\hat{F}_{A,\sigma}$, and
$\phi_g^{(s)}\sim\hat{F}_\phi$ for each gene, with rhythmicity status
$Z_g^R\sim\mathrm{Bernoulli}(\hat\pi_R)$. For rhythmic genes, generate
observations from \eqref{eq:cosinor_model}; for arrhythmic genes, set $A_g=0$.
Collection times follow the active schedule or are drawn from
$\hat{F}_{\mathrm{TOD}}$ for passive designs.

\paragraph{Active vs.\ passive designs.}
An \emph{active} design specifies the sampling schedule: the number $B$ of
distinct time points and the replication $m_b$ at each, with total $n=\sum_b
m_b$. A \emph{passive} design leaves collection times to the empirical
time-of-day distribution of the cohort (e.g., post-mortem studies). For active
designs with $B\geq 3$ equally spaced times and balanced replication,
$d(\phi)=1/2$ for all phases (Supplementary~S8), so power depends only on
total $n$. For passive designs, the design factor $d_{g} =
n^{-1}\sum_{i=1}^n\cos^2(\omega t_i - \omega\phi_g)$ varies per gene,
and \textsc{SCP} reports design-factor diagnostics to quantify how temporal
...
\paragraph{Differential endpoints.}
For two groups $k\in\{1,2\}$ and gene $g$, \textsc{SCP} supports three
differential endpoints:
\begin{itemize}
\item \textbf{DR} (differential rhythmicity): gene is rhythmic in one group
  but not the other (gain or loss of rhythmicity). Biologically, DR captures
  clock gene disruption — for example, core oscillator genes that cycle in
  healthy tissue but lose rhythmicity in tumor or neurodegenerative tissue.
\item \textbf{DP} (differential phase): gene is rhythmic in both groups but
  with a shifted acrophase, $\phi_{g,1}\neq\phi_{g,2}$. DP corresponds to
  a change in the timing of peak expression, as seen in circadian misalignment,
  shift work, or disease states that alter clock-output phasing without
  abolishing the oscillation.
\item \textbf{DM} (differential mesor): gene has the same rhythmic structure
  in both groups but a shifted baseline expression level,
  $\mu_{g,1}\neq\mu_{g,2}$. DM is the circadian analogue of differential
  expression (DE) in standard RNA-seq analysis: the gene oscillates similarly
  in both groups but at a different average level, as occurs when an
  inflammatory or disease state elevates or suppresses a gene's baseline
  while leaving its rhythmic pattern intact.
\end{itemize}
These endpoints are not mutually exclusive in real data; however, power
analysis evaluates each in isolation to produce interpretable, endpoint-specific
power curves --- the standard practice in multi-parameter power analysis.

\paragraph{Notation.}
We index genes by $g=1,\ldots,G$, subjects by $i$, groups by $k\in\{1,2\}$,
and simulation replicates by $s=1,\ldots,S$. For each endpoint
$E\in\{\mathrm{DR},\mathrm{DP},\mathrm{DM}\}$, let $z_g^E\in\{0,1\}$ indicate
a truly differential gene and $z_g^{*,E}\in\{0,1\}$ a targeted differential
gene whose effect exceeds a user-specified minimum threshold $\Delta$
(e.g., minimum amplitude ratio for DR, minimum phase shift for DP, or minimum
mesor difference for DM); formally $z_g^{*,E} = z_g^E \cdot
\mathbf{1}\{\mathrm{effect}_g^E \geq \Delta\}$. Let $D_{g,s}^E\in\{0,1\}$ indicate
discovery by the pipeline in replicate $s$. The proportions of DR, DP, and DM
genes are $\pi_{\mathrm{DR}}$, $\pi_{\mathrm{DP}}$, and $\pi_{\mathrm{DM}}$,
and $\pi_{\mathrm{R}}$ denotes the proportion of jointly rhythmic
(non-DR) genes.

\paragraph{Step II: Two-group data generation.}
Building on the pilot summary $\hat{\Psi}$ from \S\ref{sec:single_cohort}, we
extend Step~II to the two-group setting. Each gene is assigned a latent type
$T_g\in\{0,1,2,3,4,5\}$ with probabilities determined by the four user-supplied
proportions $(\pi_{\mathrm{DR}},\pi_R,\pi_{\mathrm{DP}},\pi_{\mathrm{DM}})$:
$P(T_g=2)=P(T_g=3)=\pi_{\mathrm{DR}}/2$ (DR genes split equally between
gain and loss of rhythmicity), $P(T_g=1)=\pi_R$, $P(T_g=4)=\pi_{\mathrm{DP}}$,
$P(T_g=5)=\pi_{\mathrm{DM}}$, and
$P(T_g=0)=1-\pi_{\mathrm{DR}}-\pi_R-\pi_{\mathrm{DP}}-\pi_{\mathrm{DM}}$.
\begin{itemize}
  \item $T_g=0$: arrhythmic in both groups ($A_{g,1}=A_{g,2}=0$).
  \item $T_g=1$: jointly rhythmic, no differential pattern.
  \item $T_g=2$: rhythmic in group~1 only (DR, loss of rhythmicity).
  \item $T_g=3$: rhythmic in group~2 only (DR, gain of rhythmicity).
  \item $T_g=4$: phase-shifted; $\phi_{g,2}=(\phi_{g,1}+\Delta\phi_g)\bmod 24$,
    $\Delta\phi_g\sim\mathrm{Unif}(\Delta\phi_{\min},\Delta\phi_{\max})$ (DP).
  \item $T_g=5$: differential mesor; same rhythmic structure in both groups
    ($A_{g,1}=A_{g,2}$, $\phi_{g,1}=\phi_{g,2}$) but
    $\mu_{g,2}=\mu_{g,1}+\Delta\mu_g$,
    $\Delta\mu_g\sim\mathrm{Unif}(\Delta\mu_{\min},\Delta\mu_{\max})$ (DM).
\end{itemize}
For $T_g=3$ genes (rhythmic in group~2 only), the group-2 amplitude and phase
are drawn independently from the pilot distributions:
$A_g^{*,(s)}\sim\hat{F}_{A,\sigma}$ and $\phi_g^{*,(s)}\sim\hat{F}_\phi$.

The full type-to-parameter mapping is given in Table~\ref{tab:type_mapping}.
Observations are generated from the two-group cosinor model
\begin{equation}
y_{g,k,i}^{(s)}
=
\mu_{g,k}^{(s)}
+
A_{g,k}^{(s)}\cos\!\bigl(\omega t_{k,i}^{(s)} - \omega\phi_{g,k}^{(s)}\bigr)
+
\varepsilon_{g,k,i}^{(s)},
\quad
...
\toprule
 & 0 (arrhy) & 1 (both) & 2 (g1-only) & 3 (g2-only) & 4 (DP) & 5 (DM) \\
\midrule
$A_{g,1}$ & -- & $A_g^{(s)}$ & $A_g^{(s)}$ & -- & $A_g^{(s)}$ & $A_g^{(s)}$ \\
$\phi_{g,1}$ & -- & $\phi_g^{(s)}$ & $\phi_g^{(s)}$ & -- & $\phi_g^{(s)}$ & $\phi_g^{(s)}$ \\
$A_{g,2}$ & -- & $A_g^{(s)}$ & -- & $A_g^{*,(s)}$ & $A_g^{(s)}$ & $A_g^{(s)}$ \\
$\phi_{g,2}$ & -- & $\phi_g^{(s)}$ & -- & $\phi_g^{*,(s)}$ & $(\phi_g^{(s)}+\Delta\phi_g)\bmod 24$ & $\phi_g^{(s)}$ \\
$\mu_{g,2}-\mu_{g,1}$ & 0 & 0 & 0 & 0 & 0 & $\Delta\mu_g$ \\
\bottomrule
\end{tabular}
\caption{Gene type to group-specific parameter mapping. Dashes indicate
arrhythmic groups ($A=0$). For DM genes ($T_g=5$), both groups share the same
amplitude and phase; the mesor shift $\Delta\mu_g$ creates differential
baseline expression.}
\label{tab:type_mapping}
\end{table}

\paragraph{Step III: Inference via DiffCircaPipeline.}
Given simulated dataset $\{y_{g,k,i}^{(s)}, t_{k,i}^{(s)}\}$, the
\textsc{DiffCircaPipeline} (DCP) \citep{xue2023diffcircapipeline} applies
the following three-stage procedure.

\emph{Stage 1 --- Group-level rhythmicity.} For each group $k$ and gene $g$,
fit the linearized cosinor model by OLS and compute an $F$-test
$p$-value $p_{g,k}$.

\emph{Stage 2 --- TOJR classification.} Assign each gene a Type of Joint
Rhythmicity (TOJR) label using unadjusted $p$-values:
\begin{equation}
\hat{Z}_g =
\begin{cases}
...
\label{eq:tojr}
\end{equation}
Unadjusted $p$-values are used here because TOJR assigns conditional gene pools
for downstream testing rather than making final discovery claims; BH correction
is reserved for the endpoint-specific tests below.

\emph{Stage 3 --- Endpoint testing.}
\begin{itemize}
\item \textbf{DR}: For all genes with $\hat{Z}_g\neq\texttt{arrhy}$, test
  $H_0: R_{g,1}^2 = R_{g,2}^2$, where $R_{g,k}^2 = 1 -
  \mathrm{RSS}_{g,k}/\mathrm{TSS}_{g,k}$ is the proportion of variance
  explained by the cosinor fit in group $k$, using a $\Delta R^2$ likelihood
  ratio test \citep{ding2021likelihood}, followed by BH correction across
  tested genes.
\item \textbf{DP}: For genes with $\hat{Z}_g=\texttt{both}$, test
  $H_0: A_{g,1}=A_{g,2},\,\phi_{g,1}=\phi_{g,2}$ jointly, then apply a
  post-hoc phase test with \v{S}id\'{a}k correction; BH is applied within the
  jointly rhythmic stratum.
\item \textbf{DM}: For all genes, test $H_0:\mu_{g,1}=\mu_{g,2}$ using a
  two-sample $t$-test on the OLS-estimated mesors $\hat{\mu}_{g,k}$, followed
  by BH correction across all genes. DM detection is independent of TOJR
  class, since a mesor shift can occur regardless of rhythmicity status.
\end{itemize}

\paragraph{Step IV: Power and FDR metrics.}
...
and $V_s^E=\sum_g D_{g,s}^E(1-z_g^E)$ the number of false discoveries.
The per-replicate FDR is
\[
\widehat{\mathrm{FDR}}_s^E
=
\frac{V_s^E}{R_s^E}\,\mathbf{1}\{R_s^E>0\}.
\]
Let $S_s^{*,E}=\sum_g D_{g,s}^E z_g^{*,E}$ and
$G_1^{*,E}=\sum_g z_g^{*,E}$. The per-replicate targeted power is
\begin{equation}
...
\label{eq:tpower_def}
\end{equation}
Design-level summaries are obtained by averaging across $S$ replicates.
Genes may additionally be stratified by $r_g = A_g/\sigma_g$, phase $\phi_g$,
or design efficiency $d(\phi_g)$ to reveal which classes of genes are
detectable under a given design:
\begin{equation}
...
\end{equation}
where $W_g$ is the stratification variable and $\{B_j\}$ are the strata.

%% --------------------------------------------------------------------
\subsection{Design Considerations: B vs.\ m Trade-off}
\label{sec:design_bvsm}

In active designs, investigators must allocate a fixed subject budget $n$
between the number of distinct time points $B$ and the replication level
$m = n/B$ per time point. Under the idealized cosinor model for a balanced
...
\paragraph{Two-stage plug-in baseline.}
The workflow described above constitutes a \emph{two-stage plug-in} procedure:
Step~I fits pilot parameters once, and Steps~II--IV are evaluated over a design
grid treating those estimates as fixed. For endpoint $E$, the resulting
power curve is
$\widehat{\mathrm{Power}}_{\mathrm{TS}}^{E}(n)
= S^{-1}\sum_{s=1}^{S}\widehat{\mathrm{TPower}}_s^{E}(n)$,
and the minimum recommended sample size is
$\hat{n}_{80}^{E} = \min\{n: \widehat{\mathrm{Power}}_{\mathrm{TS}}^{E}(n)\geq 0.80\}$.
This plug-in estimate is fast but treats pilot uncertainty as zero, which can
lead to over-optimistic projections when the pilot is small ($n_0 < 20$) or
the signal is weak ($\tilde{r} < 0.5$).

\paragraph{Bootstrap procedure.}
To quantify pilot uncertainty, \textsc{SCP} augments the plug-in analysis
with a nonparametric bootstrap over the fitted pilot parameter table. For each
bootstrap replicate $b=1,\ldots,R_{\mathrm{boot}}$:
\begin{enumerate}
\item Draw $G_0$ gene-level rows $\hat{\Theta}_g=(\hat{\mu}_g,\hat{A}_g,\hat{\sigma}_g,\hat{\phi}_g,\hat{Z}_g^R)$
  with replacement to form $\widehat{\Theta}^{(b)}$.
\item Reconstruct bootstrap empirical distributions
  $(\hat{F}_\mu^{(b)}, \hat{F}_{A,\sigma}^{(b)}, \hat{F}_\phi^{(b)})$.
\item Run the data generation and inference pipeline (Steps~II--IV) over the
  design grid using bootstrap inputs, averaging over $S_{\mathrm{inner}}$ inner
  simulation replicates to obtain
...
which all marginal simulator inputs are induced. The collection-time
distribution $\hat{F}_{\mathrm{TOD}}$ is held fixed, so the bootstrap
quantifies gene-level parameter uncertainty conditional on the observed
sampling schedule.

\paragraph{Inference from bootstrap replicates.}
The bootstrap mean power and pointwise $(1-\alpha_{\mathrm{CI}})$ confidence
interval at design point $(n,B)$ are
\[
\bar{P}^{E}(n,B)
=
...
\bigr],
\]
where $\widehat{\mathrm{Power}}^{E}_{q}$ is the $q$-th empirical quantile
