...
$n_{\text{pilot}} = 45$, passive; $\tilde{r} \approx 0.65$), which brackets
...
[TODO: 2--3 sentences summarising Fig 1 once final numbers confirmed.]
...
[TODO: 2--3 sentences summarising Fig 2 once final numbers confirmed.]
...
[TODO: quantitative summary once D1 N≤96 results available.]

% =====================================================================
\subsection{Waveform violation and \textsc{RAIN}: $\alpha_2$ sweep}
\label{sec:alpha2_results}

% --- FIGURE 4 PLACEHOLDER ---
% Source script: 15c_bvsm_rain_extended.R (rain_ext5k screen running)
% Will produce: results_RAIN_ext_*.rds for LIV, LUN, D1 × α₂ ∈ {0,0.5,0.75,1.0}

\begin{figure}[H]
\centering
\fbox{\parbox{0.95\textwidth}{\centering\vspace{3cm}
  FIGURE 4 PLACEHOLDER\\[6pt]
  \textsc{RAIN} + \textsc{MH} power vs $B$ and $\alpha_2$. Primary dataset:
  Mouse D1. Pending: rain\_ext5k screen.
  \vspace{3cm}}}
\caption{Effect of waveform shape ($\alpha_2$) on detection power for \textsc{RAIN}
and \textsc{MH}. \textbf{(A)} $\alpha_2 = 0$ (pure sinusoid): \textsc{RAIN}
and \textsc{MH} show comparable performance. \textbf{(B)} $\alpha_2 = 0.5$,
\textbf{(C)} $\alpha_2 = 1.0$: \textsc{MH} gains $>$[TODO]\,pp over
\textsc{RAIN} at $B \geq 6$, driven by the extra harmonic captured when
$K \geq 2$. The benefit of denser time-point coverage is visible only when the
waveform departs from sinusoidal, and only at low-to-moderate $\tilde{r}$.}
\label{fig:alpha2_rain}
\end{figure}

[TODO: fill in numbers once rain\_ext5k completes.]
...
[TODO: fill in once boot\_sc completes.]
