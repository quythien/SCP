...
differential circadian analysis, where no closed-form solution exists because
the multi-stage inference pipeline creates intractable dependence between
rhythmicity screening and endpoint testing, \textsc{SCP} evaluates power and
FDR via pilot-data-driven simulation.
...
\item \textbf{Simulation-based (recommended):} Using the pilot-estimated
  empirical distributions, simulate single-group datasets, apply the $F$-test
  with BH correction, and aggregate empirical power and FDR across simulation
  replicates (Step~I and the data simulation step below, applied to a single
  group). This naturally
  accounts for multiplicity correction, passive design, and the full
  heterogeneity of $(A_g, \sigma_g)$ jointly.
\end{enumerate}
...
disease vs.\ control), single-cohort power calculations are insufficient.
Differential circadian inference is multi-stage: the TOJR classifier assigns
each gene to a conditional pool based on per-group $p$-values, and endpoint
tests are then applied within those data-derived strata. This conditioning
creates multi-stage dependence between rhythmicity screening and downstream
testing that renders the joint power distribution analytically intractable.
\textsc{SCP} therefore evaluates operating characteristics via calibrated
