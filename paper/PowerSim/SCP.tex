...
\textsc{SCP} addresses these limitations by estimating gene-level parameters
directly from a pilot dataset. For each of the $G_0$ pilot genes, we fit
...
Two modes of single-cohort power analysis are supported. In the closed-form
mode, \eqref{eq:circapower_formula} is applied gene-by-gene using each pilot
gene's estimated $\hat{r}_g = \hat{A}_g/\hat{\sigma}_g$ and design factor
$\hat{d}(\hat{\phi}_g)$, and the results are averaged across genes to yield
$\widehat{\mathrm{Power}}_{\mathrm{CF}}(n)$. This is computationally efficient
and extends \textsc{CircaPower} to heterogeneous $r$, but does not model
genome-wide FDR. The simulation-based
mode is therefore recommended: single-group datasets are simulated from the
pilot-estimated empirical distributions, the $F$-test with BH correction is
applied, and empirical power and FDR are aggregated across replicates.
Simulation naturally captures genome-wide multiplicity correction, the full
joint heterogeneity of $(A_g, \sigma_g)$, and passive-design timing
variability — features inaccessible to the closed-form approach.

The pilot dataset is summarized by constructing empirical distributions over
the fitted gene-level parameters:
\begin{equation}
\hat{F}_\mu
=
...
specified separately. The full pilot summary is denoted
...
For each simulation replicate $s$ and candidate sample size $n$, $G$ genes are
generated by drawing $\mu_g^{(s)}\sim\hat{F}_\mu$,
$(A_g^{(s)},\sigma_g^{(s)})\sim\hat{F}_{A,\sigma}$, and
$\phi_g^{(s)}\sim\hat{F}_\phi$, with rhythmicity status
$Z_g^R\sim\mathrm{Bernoulli}(\hat\pi_R)$. For rhythmic genes, observations are
generated from \eqref{eq:cosinor_model}; for arrhythmic genes, $A_g$ is set to
zero. An \emph{active} design specifies the sampling schedule: the number $B$
of distinct time points and the replication $m_b$ at each, with total
$n=\sum_b m_b$. A \emph{passive} design leaves collection times to the
empirical time-of-day distribution of the cohort --- for example, post-mortem
studies or observational cohorts in which clinical timing governs sample
collection. For active designs with $B\geq 3$ equally spaced times and
balanced replication, $d(\phi)=1/2$ for all phases (Supplementary~S8), so
power depends only on total $n$. For passive designs, the design factor
$d_{g} = n^{-1}\sum_{i=1}^n\cos^2(\omega t_i - \omega\phi_g)$ varies per
gene, and \textsc{SCP} reports design-factor diagnostics to quantify how
temporal coverage affects efficiency.
...
but not the other. Biologically, DR corresponds to clock gene disruption ---
for example, core oscillator genes that cycle in healthy tissue but lose
rhythmicity in tumor or neurodegenerative tissue. Differential phase (DP)
indicates a shifted acrophase, $\phi_{g,1}\neq\phi_{g,2}$, arising when the
timing of peak expression changes, as seen in circadian misalignment, shift
work, or disease states that alter clock-output phasing without abolishing the
oscillation. Differential mesor (DM) captures a shifted baseline expression
level, $\mu_{g,1}\neq\mu_{g,2}$, while the rhythmic structure is otherwise
preserved; DM is the circadian analogue of differential expression in standard
RNA-seq analysis, arising when an inflammatory or disease state elevates or
suppresses a gene's average level while leaving its oscillation intact. These
endpoints are not mutually exclusive in real data; power analysis evaluates
each in isolation to produce interpretable, endpoint-specific power curves.

Throughout this section, genes are indexed by $g=1,\ldots,G$, subjects by
$i$, groups by $k\in\{1,2\}$, and simulation replicates by $s=1,\ldots,S$.
For each endpoint $E\in\{\mathrm{DR},\mathrm{DP},\mathrm{DM}\}$, $z_g^E\in\{0,1\}$
indicates a truly differential gene and $z_g^{*,E}\in\{0,1\}$ a targeted
differential gene whose effect exceeds a user-specified minimum threshold
$\Delta$ (e.g., minimum amplitude ratio for DR, minimum phase shift for DP,
or minimum mesor difference for DM); formally $z_g^{*,E} = z_g^E \cdot
\mathbf{1}\{\mathrm{effect}_g^E \geq \Delta\}$. $D_{g,s}^E\in\{0,1\}$ denotes
...
and $\pi_{\mathrm{R}}$ denotes the proportion of jointly rhythmic (non-DR) genes.

Data generation for the two-group setting builds on the pilot summary
$\hat{\Psi}$ from Section~\ref{sec:single_cohort}. Each gene is assigned a latent type
...
proceeds via \textsc{DiffCircaPipeline} (DCP; \citealt{xue2023diffcircapipeline})
in three stages.
...
Study performance is summarized by computing power and FDR across simulation
replicates. For each endpoint $E\in\{\mathrm{DR},\mathrm{DP},\mathrm{DM}\}$ and
...
the per-replicate FDR is $\widehat{\mathrm{FDR}}_s^E = V_s^E/R_s^E$ when
$R_s^E>0$ and zero otherwise. Let $S_s^{*,E}=\sum_g D_{g,s}^E z_g^{*,E}$ and
