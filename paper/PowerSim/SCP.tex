...
\textsc{DCP}, $B = 4$). \textbf{(A)} Putamen schizophrenia controls ($n_\mathrm{pilot} = 28$, passive):
the plug-in projects 28\% power at $N = 40$ while the bootstrap mean is 43\%, and
the 95\% CI reaches a maximum width of 20.5\,pp at $N = 40$, the strongest signal
of pilot-induced uncertainty across the four panels. \textbf{(B)} Seney ACC
controls ($n_\mathrm{pilot} = 60$, passive): maximum CI width ${\sim}10$\,pp at
$N = 60$. \textbf{(C)} GTEx Pancreas ($n_\mathrm{pilot} = 249$): maximum CI width
6.8\,pp at $N = 160$. \textbf{(D)} GTEx Thyroid ($n_\mathrm{pilot} = 416$):
maximum CI width 6.4\,pp at $N = 120$.
