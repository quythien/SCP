...
\textsc{CircaPower} calculation to the transcriptome-wide setting using
pilot-calibrated gene-level effect sizes. For differential circadian analysis,
where no closed-form solution exists, \textsc{SCP} uses pilot-data-driven
simulation to evaluate power and FDR under the full multi-stage inference
pipeline. Both modes follow the prospective simulation philosophy of
\textsc{PROPER} \citep{wu2015proper}, reporting power stratified by
signal-to-noise ratio rather than a single marginal average.
...
\item \textbf{Simulation-based (recommended):} Using the pilot-estimated
  ...
  replicates (Steps~I--II below, applied to a single group). This naturally
...
\paragraph{Step II: Single-group data simulation.}
...
Differential circadian inference is multi-stage and genome-wide: it requires
rhythmicity screening, differential testing for each endpoint, and
multiplicity correction. Because this pipeline has no tractable closed-form
power expression, \textsc{SCP} evaluates operating characteristics via
...
\item \textbf{DR} (differential rhythmicity): gene is rhythmic in one group
  but not the other (gain or loss of rhythmicity).
...
\item \textbf{DM} (differential mesor): gene has the same rhythmic structure
  ...
  expression (DE) in standard RNA-seq analysis.
\end{itemize}
...
gene whose effect exceeds a threshold. Let $D_{g,s}^E\in\{0,1\}$ indicate
...
$T_g\in\{0,1,2,3,4,5\}$ using proportions
...
\begin{itemize}
  ...
  \item $T_g=2$: rhythmic in group~1 only (DR).
  \item $T_g=3$: rhythmic in group~2 only (DR).
...
\item \textbf{DR}: For all genes with $\hat{Z}_g\neq\texttt{arrhy}$, test
  $H_0: R_{g,1}^2 = R_{g,2}^2$ by a $\Delta R^2$ likelihood ratio test
  \citep{ding2021likelihood}, followed by BH correction across tested genes.
...
\item Run Steps~II--IV over the design grid using bootstrap inputs, yielding
