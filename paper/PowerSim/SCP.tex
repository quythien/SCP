...
This is computationally efficient but does not propagate genome-wide FDR or the
joint $(A_g,\sigma_g)$ heterogeneity. The simulation-based pipeline is therefore
recommended and proceeds as follows.
...
with rhythmicity status $Z_g^R\sim\mathrm{Bernoulli}(\hat\pi_R)$. Observations
...
only on total $n$; for passive designs $d_g = n^{-1}\sum_i\cos^2(\omega t_i - \omega\phi_g)$
varies per gene, and \textsc{SCP} reports design-factor diagnostics to
...
With $Z_g^R\in\{0,1\}$ the true rhythmicity status assigned in Step~2, empirical
power is $\widehat{\mathrm{Power}}(n) = S^{-1}\sum_s\bigl(\sum_g D_{g,s}Z_g^R\bigr)/\bigl(\sum_g Z_g^R\bigr)$,
...
The notation $D_{g,s}$ and $Z_g^R$ introduced here extends directly to the
...
$D_{g,s}^E\in\{0,1\}$, and the single-group true-status indicator $Z_g^R$ is
replaced by $z_g^E\in\{0,1\}$ (truly differential for endpoint $E$). To
evaluate power at a minimum biologically relevant effect size, a targeted
indicator $z_g^{*,E} = z_g^E\cdot\mathbf{1}\{\mathrm{effect}_g^E \geq \Delta\}$
selects genes exceeding a user-specified threshold $\Delta$ (e.g., minimum
amplitude ratio for DR, minimum phase shift for DP, or minimum mesor difference
for DM). The proportions of DR, DP, DM, and jointly rhythmic genes are denoted
$\pi_{\mathrm{DR}}$, $\pi_{\mathrm{DP}}$, $\pi_{\mathrm{DM}}$, and $\pi_R$.

The differential pipeline follows the same four steps as Section~\ref{sec:single_cohort},
with Steps~1 and 4 unchanged and Steps~2--3 extended to the two-group setting.

\emph{Step 2 --- Two-group data generation.}
Each gene is assigned a latent type $T_g\in\{0,1,2,3,4,5\}$ drawn from the
user-supplied proportions $(\pi_{\mathrm{DR}},\pi_R,\pi_{\mathrm{DP}},\pi_{\mathrm{DM}})$.
Arrhythmic genes ($T_g=0$, probability
$1-\pi_{\mathrm{DR}}-\pi_R-\pi_{\mathrm{DP}}-\pi_{\mathrm{DM}}$) have $A_{g,1}=A_{g,2}=0$.
Jointly rhythmic genes ($T_g=1$, probability $\pi_R$) share the same amplitude and phase
in both groups. DR genes are split equally between loss of rhythmicity
($T_g=2$, group-1 rhythmic only) and gain of rhythmicity ($T_g=3$, group-2 rhythmic only),
each with probability $\pi_{\mathrm{DR}}/2$; for $T_g=3$ genes, the group-2 amplitude
and phase are drawn independently as $A_g^{*,(s)}\sim\hat{F}_{A,\sigma}$ and
$\phi_g^{*,(s)}\sim\hat{F}_\phi$. Phase-shifted genes ($T_g=4$, probability $\pi_{\mathrm{DP}}$)
have $\phi_{g,2}=(\phi_{g,1}+\Delta\phi_g)\bmod 24$ with
$\Delta\phi_g\sim\mathrm{Unif}(\Delta\phi_{\min},\Delta\phi_{\max})$.
Differential-mesor genes ($T_g=5$, probability $\pi_{\mathrm{DM}}$) share the same
amplitude and phase in both groups but $\mu_{g,2}=\mu_{g,1}+\Delta\mu_g$ with
...
correction. For DP, genes with $\hat{Z}_g=\texttt{both}$ are tested jointly
