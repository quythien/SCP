...
distribution with noncentrality $\lambda_g$ \citep{zong2023circapower}.
...
% for Section~\ref{sec:design_bvsm} and Supplementary~S8.
...
Section~\ref{sec:design_bvsm} and Supplementary~S8.
...
% CHANGE: Original paragraph introduced CircaPower's limitation and then the
% CF shortcut in the same breath before the four steps, interrupting the
% pipeline narrative. The limitation motivation is kept here; the CF shortcut
% description is moved to after Step 4 where it reads as a natural alternative.
...
with rhythmicity status $Z_{g,s}^R\sim\mathrm{Bernoulli}(\hat\pi_R)$.
Observations for rhythmic genes are generated from \eqref{eq:cosinor_model};
arrhythmic genes have $A_g=0$. An \emph{active} design specifies $B$ distinct
time points with $m_b$ replicates each and total $n=\sum_b m_b$; a \emph{passive}
design draws collection times from $\hat{F}_{\mathrm{TOD}}$, as in post-mortem
cohorts where clinical timing governs sampling. For balanced equally spaced
active designs with $B\geq 3$, $d(\phi)=1/2$ for all phases
(Supplementary~S8), so power depends only on total $n$; for passive designs
the realized $d(\phi_g)$ is computed from the observed collection times via
\eqref{eq:design_factor_circapower}, varies by gene and replicate, and
\textsc{SCP} reports design-factor diagnostics to quantify how temporal
coverage affects efficiency.
...
% CHANGE: Notation forward-reference kept here but simplified. The explicit
% connection to the differential setting is now made by the opening sentence
% of Section~\ref{sec:differential}, reducing redundancy.
...
% CHANGE: Notation extension for D, Z moved to the opening of this section
% so that readers have the full symbol map before encountering the differential
% steps, avoiding forward-reference confusion.
...
For each endpoint $E\in\{\mathrm{DR},\mathrm{DP},\mathrm{DM}\}$, $D_{g,s}^E\in\{0,1\}$
denotes the endpoint-specific discovery indicator and $Z_{g,s}^E\in\{0,1\}$
denotes true differential status for that endpoint. A targeted indicator
$Z_{g,s}^{*,E} = Z_{g,s}^E\cdot\mathbf{1}\{\delta_{g,s}^E \geq \Delta_E\}$
further restricts to genes whose effect size exceeds a user-specified
threshold $\Delta_E$, where $\delta_{g,s}^{\mathrm{DR}}$ is the simulated
rhythmicity effect size, $\delta_{g,s}^{\mathrm{DP}}$ is the absolute phase
shift, and $\delta_{g,s}^{\mathrm{DM}}$ is the absolute mesor difference.
...
\textsc{SCP} evaluates three differential endpoints. Differential rhythmicity
(DR) captures gain or loss of rhythmicity: the gene is rhythmic in one group
but not the other \citep{Singer2019LimoRhyde,ding2021likelihood,xue2023diffcircapipeline}.
Biologically, DR corresponds to clock gene disruption --- for example, core
oscillator genes that cycle in healthy tissue but lose rhythmicity in disease
states \citep{takahashi2017transcriptional,chen2016aging,seney2019diurnal}.
Differential phase (DP) indicates a shifted acrophase, $\phi_{g,1}\neq\phi_{g,2}$,
arising when the timing of peak expression changes
\citep{parsons2020circacompare,ding2021likelihood}. Differential mesor (DM)
captures a shifted baseline expression level, $\mu_{g,1}\neq\mu_{g,2}$, while
the rhythmic structure is otherwise preserved
\citep{parsons2020circacompare,Singer2019LimoRhyde}. Although these endpoints
may co-occur in real data, \textsc{SCP} evaluates them in separate simulation
scenarios to produce interpretable, endpoint-specific power curves
\citep{zong2023circapower}. The proportions of DR, DP, DM, and jointly
rhythmic genes are denoted $\pi_{\mathrm{DR}}$, $\pi_{\mathrm{DP}}$,
$\pi_{\mathrm{DM}}$, and $\pi_R$.

The differential pipeline follows the same four-step structure as
Section~\ref{sec:single_cohort}. Step~1 is shared; Steps~2--4 are adapted to
the two-group setting. In each endpoint-specific simulation scenario, genes are
assigned mutually exclusive latent states so that power for the target endpoint
can be evaluated cleanly without overlap from other differential alternatives.

\emph{Step 2 --- Two-group data generation.}
% CHANGE: One-pilot case is now the primary narrative path because it is the
% more common default; two-pilot is demoted to a clearly labeled subordinate
% paragraph. This removes the conditional branching that interrupted the
% parameter-assignment description.
\textsc{SCP} begins from pilot-derived baseline parameter distributions. In the
common case where only one pilot group is available, a single reference summary
$\hat{\Psi}=(\hat{F}_{\mu},\hat{F}_{A,\sigma},\hat{F}_{\phi},\hat{F}_{\mathrm{TOD}})$
is used to draw a baseline parameter tuple
$(\mu_g^{(s)},A_g^{(s)},\sigma_g^{(s)},\phi_g^{(s)})$
with $\mu_g^{(s)}\sim\hat{F}_{\mu}$,
$(A_g^{(s)},\sigma_g^{(s)})\sim\hat{F}_{A,\sigma}$, and
$\phi_g^{(s)}\sim\hat{F}_{\phi}$.
This baseline defines group~1 parameters
$(\mu_{g,1}^{(s)}, A_{g,1}^{(s)}, \sigma_{g,1}^{(s)}, \phi_{g,1}^{(s)})$;
group~2 parameters are then obtained by applying the endpoint-specific
perturbations described below. Under this one-pilot parameterization the two
groups share the same baseline noise level unless heteroscedasticity is
explicitly modeled.

\emph{Two-pilot extension.} When pilot data are available for both groups,
group-specific pilot summaries
$\hat{\Psi}_k=(\hat{F}_{\mu,k},\hat{F}_{A,\sigma,k},\hat{F}_{\phi,k},\hat{F}_{\mathrm{TOD},k})$
for $k\in\{1,2\}$ are estimated and baseline parameters are drawn separately
for each group:
\[
...
proportions $(\pi_{\mathrm{DR}},\pi_R,\pi_{\mathrm{DP}},\pi_{\mathrm{DM}})$
that are either user-specified or estimated from pilot data. In the one-pilot
setting, all four proportions are user-specified. In the two-pilot setting,
$\pi_{\mathrm{DR}}$ is estimated as the fraction of genes rhythmic in exactly
one group, and $\pi_{\mathrm{DP}}$ is estimated as the fraction of jointly
rhythmic genes with an observed phase shift exceeding a user-specified
threshold; $\pi_{\mathrm{DM}}$ is always user-specified, since mesor
differences are not reliably identifiable from a pilot dataset used to
calibrate the baseline distributions. Regardless of their origin, the
proportions must satisfy
$\pi_{\mathrm{DR}}+\pi_R+\pi_{\mathrm{DP}}+\pi_{\mathrm{DM}}\leq 1$,
with the remainder assigned to arrhythmic genes ($T_g=0$).
...
$A_{g,1}=0$. Phase-shifted genes ($T_g=4$, probability $\pi_{\mathrm{DP}}$)
retain the same mesor, amplitude, and noise level but have
$\phi_{g,2}=(\phi_{g,1}+\Delta\phi_g)\bmod 24$ with
...
same amplitude, phase, and noise level but have
...
Observations in each group are generated from \eqref{eq:cosinor_model},
applied separately to groups $k\in\{1,2\}$ with group-specific parameters
$(\mu_{g,k}^{(s)},A_{g,k}^{(s)},\phi_{g,k}^{(s)},\sigma_{g,k}^{(s)})$
and collection times $t_{k,i}^{(s)}$.

\emph{Step 3 --- DCP inference.}
% CHANGE: Added a one-sentence map at the top of Step 3 so readers have an
% orientation before the three sub-stages expand.
Inference proceeds via \textsc{DiffCircaPipeline} (DCP;
\citep{xue2023diffcircapipeline}) in three sub-stages: per-group rhythmicity
screening, Type of Joint Rhythmicity (TOJR) classification, and
endpoint-specific testing.

First, for each group $k$ and gene $g$, the linearized cosinor model is fit by
OLS and an $F$-test $p$-value $p_{g,k}$ is computed. Second, each gene is
assigned a TOJR label using unadjusted $p$-values:
\begin{equation}
\hat{Z}_g =
\begin{cases}
...
\label{eq:tojr}
\end{equation}
Unadjusted $p$-values are used because TOJR assigns conditional gene pools for
downstream testing rather than making final discovery claims; BH correction is
reserved for the endpoint-specific tests. Third, endpoint-specific tests are
...
\textsc{DiffCircaPipeline} test statistic. For DP, genes with
$\hat{Z}_g=\texttt{both}$ are tested jointly for
$H_0: A_{g,1}=A_{g,2},\,\phi_{g,1}=\phi_{g,2}$, with a post-hoc phase test
applying \v{S}id\'{a}k correction and BH applied within the jointly rhythmic
stratum. For DM, all genes are tested for $H_0:\mu_{g,1}=\mu_{g,2}$ via a
two-sample $t$-test on OLS-estimated mesors $\hat{\mu}_{g,k}$, followed by BH
correction across all genes; DM detection is TOJR-independent since a mesor
shift can occur regardless of rhythmicity status.
...
The per-replicate targeted power is then the proportion of above-threshold true
...
% CHANGE: B-invariance is no longer re-derived here. Instead it opens with a
% forward reference back to Section~\ref{sec:single_cohort}, which is where the
% reader first encountered the result. This removes the redundancy while
% keeping Section~\ref{sec:design_bvsm} focused on when invariance breaks down.
As noted in Section~\ref{sec:single_cohort}, balanced equally spaced designs
with $B\geq 3$ time points satisfy the \emph{B-invariance} property: the
design factor $d(\phi)=1/2$ for all gene phases, so the noncentrality parameter
reduces to $\lambda_g = r_g^2 n / 2$ and power depends on total $n$ but not
on the $B$-vs-$m$ allocation (Supplementary~S8).
...
correction, all of which interact with the design in non-trivial ways. At low
signal-to-noise ratios, denser time sampling ($\uparrow B$, $\downarrow m$)
...
(Supplementary~S7). \textsc{SCP} evaluates the $B$-vs-$m$ trade-off directly
by sweeping both $B$ and $n$ in the simulation grid, producing joint power
surfaces that reveal whether the allocation matters for a given pilot dataset
and target endpoint.
...
pilot parameters are estimated once from the data, and the simulation grid is
then evaluated treating those estimates as fixed.
% CHANGE: Practical guidance on when the plug-in is insufficient moved earlier,
% immediately after defining the plug-in, so readers know the motivation
% before the bootstrap solution is introduced.
Although computationally efficient, this plug-in estimate treats pilot
uncertainty as zero, which can lead to over-optimistic projections when the
pilot is small ($n_0 < 20$) or the signal is weak ($\tilde{r} < 0.5$).
For endpoint $E$, the resulting power curve is
...
across bootstrap replicates. Narrow intervals indicate stable power projections;
wide intervals signal that the pilot is insufficiently informative to support a
precise sample-size recommendation, and a larger pilot or additional assumptions
are warranted.
%%
