...
\section{Proof of Proposition~1 (B-invariance)}
\label{sec:proof_b_invariance}

Under balanced equally spaced sampling with $B\ge 3$ distinct time points and
common replication $m$ per time point, so that $n=Bm$, the Fisher information
block for $(\beta_1,\beta_2)$ is $(n/2)I_2$ by \eqref{eq:xtx_balanced}.
The noncentrality parameter for the global rhythmicity test
...
equally spaced grid (Section~3), so
\[
d(\phi)=\frac{1}{2}
\]
for all $\phi$.\qed
