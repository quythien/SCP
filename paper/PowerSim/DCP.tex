...
Power analysis in circadian transcriptomics poses challenges that do not arise in simpler settings. One-group circadian power calculations---answering ``how many samples to detect a rhythmic gene?''---are well established and addressed by tools such as \software{CircaPower} \cite{zong2023circapower}. The differential circadian problem is fundamentally harder: the inference pipeline is multi-stage (rhythmicity screening, DR or DP testing, multiplicity correction), closed-form power expressions are unavailable, and the relevant effect size---the amplitude-to-noise ratio $r=A/\sigma$---is not fixed but varies across tens of thousands of genes in a transcriptome-wide study. A single representative $r$ (e.g., from core clock genes, which typically have $r\approx 3$--$6$ in mouse liver) is $4$--$10\times$ larger than the median $r$ across all expressed genes in human brain tissue ($\tilde{r}\approx 0.6$), so using it in a power calculation produces severely over-optimistic projections for most of the transcriptome. Simulation calibrated to the full empirical $(A,\sigma)$ distribution is therefore the natural approach.

This fixed-$r$ simplification becomes especially limiting in transcriptomic differential circadian studies, where amplitude and noise vary substantially across genes and where inference is applied through a multi-stage two-group pipeline rather than a single-gene analytical test. In practice, a single representative $r$ can obscure the fact that some genes are essentially undetectable while others are well powered under the same study design.
...
\begin{itemize}
\item We extend circadian power analysis from one-group fixed-effect-size calculations to two-group differential circadian studies by simulating from pilot-derived empirical distributions of gene-level parameters, especially the joint distribution of amplitude and noise. In practice, the median $r=A/\sigma$ across all expressed genes in a transcriptome is typically $4$--$10\times$ lower than $r$ for core clock genes; assuming a single fixed $r$ therefore yields severely over-optimistic power projections for most of the transcriptome.
...
cohort (e.g., post-mortem studies). We adopt the linearized first-harmonic
representation of the cosinor model,
\begin{equation}
y_{g,k}(t)
=
\mu_{g,k}
...
We first consider balanced active designs. For equally spaced sampling times
$t_b=(b-1)\tau/B$ with $B\geq 3$ and common replication $m$ per time point
($n=Bm$), the first-harmonic basis is orthogonal on the sampled grid and the
squared-cosine and squared-sine sums each equal $B/2$ (derivation in
Supplementary~S8). Substituting into \eqref{eq:info_matrix} gives
\begin{equation}
X^\top X = \operatorname{diag}(n,\,n/2,\,n/2).
...
\begin{proof}
See Supplementary~S8.
\end{proof}

Proposition~\ref{prop:b_invariance} formalizes the design-factor result of
\citet{zong2023circapower} in the two-group differential setting: within the
restricted class of balanced active designs, reallocating effort
...
does not satisfy the orthogonality conditions of Section~\ref{sec:design_theory}. In this case we write
...
\mathcal{H}
=
\{\pi_{\mathrm{DR}},\pi_{\mathrm{R}},\pi_{\mathrm{DP}},
...
$H_0:E_g=F_g=0$, and define the rhythmic gene set as
...
$[\Delta\phi_{\min},\Delta\phi_{\max}]$.
Together, $\hat{\Psi}$ summarizes what is borrowed from the pilot; Step~II
uses these estimates to generate synthetic two-group datasets under any
candidate design.
...
$\lfloor\pi_{\mathrm{R}}|\mathcal{A}\setminus\mathcal{G}_{\mathrm{DR}}|\rfloor$
...
Each simulated dataset $\mathcal{D}^{(s)}$ is passed to Step~III, which
applies the full DCP inference pipeline to produce gene-level differential
calls.
...
$p$-values for screening because it assigns conditional gene pools for
downstream testing rather than making final discovery claims; BH correction
is reserved for the DR and DP tests that produce reported discoveries.
...
The gene-level calls from Step~III are aggregated in Step~IV to compute
power and FDR metrics for the current design.
...
variability of the resulting power curves. Because the resampling unit is the entire fitted row $\hat{\Theta}_g$,
the bootstrap preserves the empirical joint distribution of $(A,\sigma)$,
from which all marginal simulator inputs are induced. In the current
...
To evaluate \software{DiffCircaPower} across a wider range of study designs and effect sizes, we analyzed three additional pilot datasets spanning a 5-fold range in effect size. The mouse LIV vs.\ CER comparison (GSE54651; $n_{\text{pilot}} = 8$, $B = 8$, $m = 1$; Zhang et al.\ \cite{zhang2014comprehensive}) has median signal-to-noise ratio $\tilde{r} = 2.88$ and $\hat{\pi}_{\mathrm{DR}} = 29.6\%$, representing the highest-signal scenario. The baboon LUN vs.\ CER comparison ($n_{\text{pilot}} = 12$, $B = 12$, $m = 1$) has $\tilde{r} = 1.72$ and $\hat{\pi}_{\mathrm{DR}} = 40.7\%$. The mouse D1 vs.\ D2 striatal neuron comparison ($n_{\text{pilot}} = 45$, $B = 6$, $m \approx 7.5$) has $\tilde{r} = 0.65$ and $\hat{\pi}_{\mathrm{DR}} = 19.9\%$ (Table~\ref{tab:signal_summary}).
...
\midrule
Mouse LIV vs.\ CER \cite{zhang2014comprehensive} & Active, $B=8$, $m=1$ & Liver & 2.88 & 29.6\% & 8 \\
