...
\citet{zong2023circapower} in the two-group differential setting: for balanced
active designs, detection efficiency depends on total $n$, not the $B$-vs-$m$
allocation. This is a theoretical baseline; practical power under the full
multi-stage pipeline can deviate due to screening, multiplicity correction, and
effect-size heterogeneity, so we assess adherence via simulation.
...
does not satisfy the first-harmonic orthogonality conditions that underpin
Proposition~\ref{prop:b_invariance}. In this case we write
